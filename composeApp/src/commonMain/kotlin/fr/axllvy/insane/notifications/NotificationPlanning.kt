...
fun stageLabel(stage: StageKey): String =
    when (stage) {
...
fun computeScheduledNotifications(
    ...
    for (key in favKeys) {
        ...
        val artist = parts[3]
        val set =
            lineup[day]?.get(stage)?.firstOrNull { it.s == start && it.a == artist } ?: continue
...
private fun setStartEpochMs(day: DayKey, set: SetEntry): Long? =
    runCatching {
