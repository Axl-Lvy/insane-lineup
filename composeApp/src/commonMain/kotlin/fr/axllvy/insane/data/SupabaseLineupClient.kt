 ...
 * Uses the authed Bearer token from [SessionStore]; the lineup table needs an
 * `authenticated` SELECT policy.
 */
class SupabaseLineupClient(
    ...
    suspend fun fetchLineup(): Lineup? {
        ...
        val response = http.pgGet(session, path) {
        ...
        if (response.status.value !in 200..299) {
            throw LineupFetchException(
                status = response.status.value,
                bodyExcerpt = body.take(400),
            )
        }
