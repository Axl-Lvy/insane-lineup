...
/**
 * GoTrue session payload (subset). Returned by /auth/v1/signup and /auth/v1/token.
 * `expires_at` is unix-seconds and may be missing on some endpoints — `expires_in`
 * is always present, so we recompute `expiresAtMs` ourselves on every refresh.
 */
@Serializable
...
internal fun GoTrueSession.toStored(nowMs: Long): StoredSession = StoredSession(
